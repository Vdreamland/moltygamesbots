...
class StormStrategy:
    ...
    def evaluate_storm_evacuation(state: GameState, memory: WorldModel) -> Optional[MoveAction]:
        """
        Mengevaluasi apakah agen terancam oleh badai (Death Zone) aktif saat ini atau badai yang akan menutup.
        ...
        for r_id in connections:
            ...
            # [PERBAIKAN SINKRONISASI BADAI]: Wilayah tetangga dianggap bahaya jika pending badai ATAU sudah aktif badai
            is_neighbor_badai = r_id in pending_dz or memory.is_known_death_zone(r_id)

            if not is_neighbor_badai:
