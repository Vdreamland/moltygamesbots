...
class HealingStrategy:
    ...
    def evaluate_heal_retreat(state: GameState, memory: WorldModel) -> Optional[MoveAction]:
        ...
        # [PERBAIKAN JANGKAUAN]: Hanya deteksi ancaman instan jika ada musuh hidup di region murni yang sama (is_alive)
        ...
        for r_id in connections:
            ...
            # [PERBAIKAN SINKRONISASI BADAI]: Berikan penalti mutlak murni jika region terdeteksi pending atau aktif badai
            is_badai = r_id in state.pending_deathzones or memory.is_known_death_zone(r_id)
            if is_badai:
                score -= 2000.0
