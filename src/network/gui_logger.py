...
class GUILogger:
    ...
    def log_turn(state: GameState, action: Optional[Action], can_act: bool = True):
        ...
        bag_items_desc = ", ".join([f"{item.name} ({item.type})" for item in player.inventory]) if player.inventory else "None"
        ...
        # BARIS ZIRAH BARU: Menampilkan status zirah aktif yang terpasang di slot Defense
